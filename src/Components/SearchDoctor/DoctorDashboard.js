...
function DoctorDashboard() {
    ...
    useEffect(() => {
        const fetch = async () => {
            const result = await axios.get(`https://admin.susasthya.com/api/appointments/doctor/${info.user.id}`)
            setData(result.data);
        }
        fetch();
    }, [])
    ...
    const handleAccept = async(id) => {
        ...
        await axios.post(`https://admin.susasthya.com/api/appointments/accept/${id}`)
        window.location.reload()
    }
    const handleCancel = async(id) => {
        
        await axios.post(`https://admin.susasthya.com/api/appointments/cancel/${id}`)
        window.location.reload()
    }
            ...
            <tr>
                ...
                {i.status == "option2" &&
                    <>
                        ...
                        <td class="d-flex">
                            <button class="btn btn-sm bg-success-light m-2" onClick={(e) => handleAccept(i.appointment_id)}>
                                <i class="fas fa-check"></i> Accept
                            </button>
                            <button class="btn btn-sm bg-danger-light m-2" onClick={(e) => handleCancel(i.appointment_id)}>
